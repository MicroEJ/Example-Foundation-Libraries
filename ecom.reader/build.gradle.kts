...
microej {
    applicationEntryPoint = "com.microej.example.foundation.ecom.uartreader.ExampleUARTReader"
...
dependencies {
    ...
    */
    ...
    implementation(libs.api.ecom)
    implementation(libs.api.ecom.comm)
...
tasks.runOnSimulator{
    /*
    * The properties "s3.mock.uart.com0.file.input" and "s3.mock.uart.com0.file.output"
    * are set automatically.
    */
    var fileInPath =
        layout.buildDirectory.file("../sim/filein.txt").get().toString().replace("\\", "\\\\")
    var fileOutPath =
        layout.buildDirectory.file("../sim/fileout.txt").get().toString().replace("\\", "\\\\")
    systemProperties.put("microej.option.s3.mock.uart.com0.file.input", fileInPath)
    systemProperties.put("microej.option.s3.mock.uart.com0.file.output", fileOutPath)
}